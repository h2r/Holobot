...
public class TFListener : MonoBehaviour
{
	...
	void Start ()
	{
        ...
        wsc.Subscribe("coord_pub", "std_msgs/String", "none", 0);
        wsc.Advertise("unity_pub", "std_msgs/String");
    ...
    void Update() {
        ...
        //    return;
        //}
        //if (wsc.messages.Count > 0)
        //{
        //    Debug.Log("Message detected!!");
        //}
        string message;
        wsc.Publish("unity_pub", "hallo");
        try
        {
            //message = wsc.messages[topic]; //get newest robot state data (from transform)
            message = wsc.messages["coord_pub"]; //get newest robot state data (from transform)
	...
	{
        // Debug.Log(StateManager.Instance.RobotOffset);
